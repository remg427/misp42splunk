...
def create_misp_events(config, results):
    ...
    for row in results:
        ...
        # now we look for attribute belonging to a file object i.e.
        # on the same row, field(s) start(s) with fo_
        fo_list = []
        for key, value in row.items():
        ...
        # then we look for attribute belonging to an email object
        # i.e. on the same row, field(s) start(s) with eo_
        eo_list = []
        for key, value in row.items():
