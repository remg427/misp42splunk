...
class AlertActionWorkermisp_alert_sighting(ModularAlertBase):
    ...
    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkermisp_alert_sighting, self)\
    ...
    def validate_params(self):
        ...
        if not self.get_param("mode"):
            self.log_error(
                'mode is a mandatory parameter, but its value is None.'
            )
            return False

        if not self.get_param("type"):
            self.log_error(
                'type is a mandatory parameter, but its value is None.'
            )
            return False
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_misp_alert_sighting_helper.process_event(
                self,
                *args,
                **kwargs
            )
...
if __name__ == "__main__":
    exitcode = AlertActionWorkermisp_alert_sighting(
        "misp42splunk",
        "misp_alert_sighting"
    ).run(sys.argv)
    sys.exit(exitcode)
