...
class AlertActionWorkermisp_alert_create_event(ModularAlertBase):
    ...
    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkermisp_alert_create_event, self).__init__(ta_name, alert_name)
    ...
    def validate_params(self):
        ...
        if not self.get_param("distribution"):
            self.log_error('distribution is a mandatory parameter, but its value is None.')
            return False

        if not self.get_param("threatlevel"):
            self.log_error('threatlevel is a mandatory parameter, but its value is None.')
            return False

        if not self.get_param("analysis"):
            self.log_error('analysis is a mandatory parameter, but its value is None.')
            return False
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_misp_alert_create_event_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkermisp_alert_create_event("misp42splunk", "misp_alert_create_event").run(sys.argv)
    sys.exit(exitcode)
