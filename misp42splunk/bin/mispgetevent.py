...
def mispgetioc(a_item, typelist, pipesplit=False, object_id=0 , object_name="", object_comment=""):
    misp_a = dict()
...
class mispgetevent(ReportingCommand):
    ...
    .. code-block::
        ...
        | mispgetevent misp_instance=<input> date=<<YYYY-MM-DD> (date_to=<YYYY-MM-DD>)
    ##Description
    {
            ...
            for r_item in response['response']:
                if 'Event' in r_item:
                    for a in r_item.values():
                        ...
                        v['misp_publish_timestamp'] = str(a['publish_timestamp'])
                        ...
                        if 'Attribute' in a:
                            v['misp_attribute_count'] = v['misp_attribute_count'] + len(a['Attribute'])
                            if my_args['getioc'] is True:
                                for attribute in a['Attribute']:
                                    ...
                                    if int(attribute['object_id']) == 0 \
                                       and '|' in attribute['type'] and my_args['pipe'] is True:
                                        mv_type_list = attribute['type'].split('|')
                                        mv_value_list = str(attribute['value']).split('|')
                                        left_a = attribute.copy()
                                        left_a['type'] = mv_type_list.pop()
                                        left_a['value'] = mv_value_list.pop()
                                        v['Attribute'].append(mispgetioc(left_a, typelist, my_args['pipe'], \
                                                                         left_a['object_id']))
                                        right_a = attribute.copy()
                                        right_a['type'] = mv_type_list.pop()
                                        right_a['value'] = mv_value_list.pop()
                                        v['Attribute'].append(mispgetioc(right_a, typelist, my_args['pipe'], \
                                                                         right_a['object_id']))
                                    else:                                    
                                        v['Attribute'].append(mispgetioc(attribute, typelist, my_args['pipe'], \
                                                                         attribute['object_id']))
                        if 'Object' in a:
                            for misp_o in a['Object']:
                                if 'Attribute' in misp_o:
                                    v['misp_attribute_count'] = v['misp_attribute_count'] + len(misp_o['Attribute'])
                                    if my_args['getioc'] is True:
                                        object_id = misp_o['id']
                                        ...
                                        for attribute in misp_o['Attribute']:
                                            v['Attribute'].append(mispgetioc(attribute, typelist, my_args['pipe'], \
                                                                  object_id, object_name, object_comment))
                ...
                if 'Attribute' in e:
                    for r in e['Attribute']:
                        if int(r['misp_object_id']) == 0:  # not an object
                            key = str(e['misp_event_id']) + '_' + r['misp_attribute_id']
                            is_object_member = False
                        else:  # this is a  MISP object
                            key = str(e['misp_event_id']) + '_object_' + str(r['misp_object_id'])
